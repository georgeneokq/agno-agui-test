...
def run_workflow(workflow: Workflow, input: RunAgentInput) -> Iterator[BaseEvent]:
    """Run the contextual Team, mapping AG-UI input messages to Agno format, and streaming the response in AG-UI format."""
    run_id = input.run_id or str(uuid.uuid4())
    try:
        # Extract the last user message for team execution
...
def get_sync_agui_router(agent: Optional[Agent] = None, team: Optional[Team] = None, workflow: Optional[Workflow] = None) -> APIRouter:
    ...
    def _run(run_input: RunAgentInput):
        def event_generator():
            if agent:
                ...
                for event in run_workflow(workflow, run_input):
                    encoded_event = encoder.encode(event)
                    yield encoded_event
